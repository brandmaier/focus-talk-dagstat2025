...
$$\mathbf{ \Sigma = F^T \left(I_l - A \right)^{-1} S \left( I_l - A \right) F }$$
...
Run a SEM forest based on a single indicator model with two parameters (mean and variance) and compute variable importance
...
Focus parameter on regression coefficient:
...
.pull-left[
- Adapte BORUTA `r  Citep(bib, "boruta")` to SEM Tree (with Priyanka Paul & Timothy Brick, PennState)
- BORUTA is a heuristic to determine a cut-off between important and not-important variables 
]

.pull-right[
<center><img src="800px-Boruta.jpg" style="height:400px" /></center>
...
# Outlook I (cont'd)
.small[
- *Shadow Features Creation*: duplicates of the original features with their values shuffled
- *SEM Forest Training*: SEM forest with both the original and shadow features. Evaluate importance metric.
- *Importance Heuristic*: compare the importance of each original feature to the maximum importance of the shadow features.
- *Decision Making*:
  - Confirmed: Feature is consistently better than shadows; keep feature 
  - Rejected: Features that consistently worse; remove feature
  - Tentative: All others
- *Repeat*
...
(Joint) Ordinal models using threshold models (example: mammography screening experience and opinions from `r Citep(bib, "hothorn2006unbiased")`)
...
- For specialized models, `mob/partykit` is much more efficient
