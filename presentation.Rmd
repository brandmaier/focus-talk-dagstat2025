...
.pull-left[
![:scale 50%](img/csp.png)
]
.pull-right[
- Score based test statistics are functions of the case-wise derivatives of the (log)likelihood function
- They form the backbone of MOB in `party/partykit` and were also brought to SEM trees by @arnold2021score
]
---
...
Let's simulate some Gaussian data and two predictors `pred_mean` and `pred_var` that perfectly predict differences in either location or scale. 

![:scale 20%](img/simulated_four_univ.png)
...
---
# Inspect first tree
Tree #1 from a forest without focus parameters
```{r}
...
- "Lack of evidence of measurement invariance equivocates conclusions and casts
doubt on theory in the behavioral sciences" `r Citet(bib, "horn1992practical")`
- Measurement Invariance is usually based on multigrop SEM/CFA `r Citet(bib, "marsh2014exploratory")`
- SEM Trees were suggested as a tool for measurement invariance testing, that is, to explore differential item functioning `r Citet(bib, c("finch2017structural","sterner2023exploratory"))`
- CFA partitioning can even be used without any groups at all `r Citet(bib, c("merkle2013tests"))`  
...
---
# BFI
- With focus parameters, specify a CFA
- choose factor loadings as focus parameters
