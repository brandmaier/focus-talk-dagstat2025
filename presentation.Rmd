...
(an instance of model-based recursive partitioning `r Citet(bib, c("zeileis2008model"," brandmaier2013structural"))` )
...
`r Citet(bib, c("brandmaier2013structural"))` proposed  old-school likelihood ratio test, given a parametric model $M$ with ML parameter estimates $\hat{theta}$ and data $X$, which is exhaustively split into $X_1$ and $X_2$ with corresponding maximum likelihood estimates $\hat{theta}_1$ and $\hat{theta}_2$:
--

$$LR =-2LL \left( X_{1}|M(\hat{\theta}_1))-2LL(X_{2}|M(\hat{\theta}_2)) + 2LL(X|\hat{\theta}) \right)$$
which is asymptotically $\chi^2$-distributed if H0 is true (i.e., no group differences) with $df = dim(\theta)$
---
...
.pull-left[
- Likelihood ratio tests are computationally expensive, we get convergence problems, naive LR test suffer from multiple testing issues and variable selection bias  `r Citep(bib, c("strobl2007bias"))` 
...
$$LR =-2LL \left( X_{1}|M(\hat{\theta}_1))-2LL(X_{2}|M(\hat{\theta}_2)) + 2LL(X|\hat{\theta}) \right)$$
we estimate _loss of fit due to constraining only focus parameters to identity_:

$$ LR =-2LL \left( X_{1}|M(\theta_1))-2LL(X_{2}|M(\theta_2) \right) -2LL \left( X_{1}|M(\theta'_1))-2LL(X_{2}|M(\theta'_2)) \right) $$
...
Run a SEM forest based on a single indicator model with two parameters.

```{r}
#vim2<-readRDS("data/03_vim2.rds")
#vimc<-readRDS("data/03_vimc.rds")
```
---
# Inspect first tree
...
- For each tree, for every observation $x$ in the _OOB_, we compute the (-2log)likelihood of observing $x$ given the leaf found before and after permuting predictor $p$
---
# Problem
...
Here, we find differences w.r.t. all parameters (e.g., predictor mean and variance _MEAN\\_lpc_ and _VAR\\_lpc_) and also a large range of regression coefficients $b_1$ ranging from $min(param_leafs["b1",])$ to $max(param_leafs["b1",])$.
```{r}
...
- `r Citet(bib, c("finch2017structural"))`  looked at a _attitudes towards reading items_ questionnaire
...
.pull-left[
![:scale 35%](img/finch-tree-nations.png)
]

.pull-right[
> SEMtree does not isolate specific differences among model parameters, but rather identifies differences in whole patterns of model parameters `r Citet(bib, "finch2017structural")`
]
...
-  International Personality Item Pool from `psych` package
- $n=2,800$
- 25 personality self report items (representing 5 OCEAN factors)
- Three demographic variables as predictors
...
---
# BFI Loadings
![](img/06bfi-tree-loadings.png)
f3: Know how to captivate people
f5: Take charge
---
# BFI Forest
